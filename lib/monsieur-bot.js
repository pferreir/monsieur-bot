...
_.extend(Bot.prototype, {
  ...
  initialize: function() {
    ...
    // Listen to PING
    this.cl.on('iq', function(stanza) {
      if (stanza.attrs.type == 'get' && stanza.getChild('ping')) {
        utils.log.info("PING? PONG!");

        this.cl.send(new xmpp.Element('iq', {
          from: stanza.attrs.to,
          to: stanza.attrs.from,
          type: 'result',
          id: 'pong' + new Date().getTime()
        }));
      }
    });
  },
